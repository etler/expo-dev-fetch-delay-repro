 ...
 * @param config
 */
function withAndroidExpoSSEPatch(config) {
  return withDangerousMod(config, [
    ...
    async (config) => {
      ...
      const expoModuleCorePath = path.join(
        projectRoot,
        "node_modules",
        "expo-modules-core"
      );
      const expoModuleCoreExists = await directoryExistsAsync(
        expoModuleCorePath
      );
      ...
      if (start < 0) {
        throw new Error(
          "SSE_PATCH_ERR: Could not find interceptor break condition"
        );
      }
